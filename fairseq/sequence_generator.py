...
from fairseq.models import FairseqIncrementalDecoder, NonAutoRegCharGenerator
...
class NonAutoRegCharSequenceGenerator(SequenceGenerator):
    def __init__(
        self, models, tgt_dict, beam_size=1, minlen=1, maxlen=None, stop_early=True,
        normalize_scores=True, len_penalty=1, unk_penalty=0, retain_dropout=False,
        sampling=False, sampling_topk=-1, sampling_temperature=1,
        ...
        super().__init__(models, tgt_dict, beam_size, minlen, maxlen, stop_early,
    ...
    def _generate(self, encoder_input, beam_size=None, maxlen=None, prefix_tokens=None):
        ...
        def finalize_hypos(step, bbsz_idx, eos_scores, unfinalized_scores=None):
            ...
            sents_seen = set()
            # XXX: (shuoyang) iterating through the batches?
            for i, (idx, score) in enumerate(zip(bbsz_idx.tolist(), eos_scores.tolist())):
                unfin_idx = idx // beam_size
                sent = unfin_idx + cum_unfin[unfin_idx]
        ...
        for step in range(maxlen + 1):  # one extra step for EOS marker
            ...
            scores = scores.type_as(lprobs)  # (batch_size * beam_size, max_len + 1)
            scores_buf = scores_buf.type_as(lprobs)
            eos_bbsz_idx = buffer('eos_bbsz_idx')
            eos_scores = buffer('eos_scores', type_of=scores)
            if step < maxlen:
                if prefix_tokens is not None and step < prefix_tokens.size(1):
                    ...
                    cand_scores, cand_indices, cand_beams = self.search.step(
                        step,
                        lprobs.view(bsz, -1, word_per_hypothesis),
                        scores.view(bsz, beam_size, -1)[:, :, :step],
                    )
            else:
                # make probs contain cumulative scores for each hypothesis
